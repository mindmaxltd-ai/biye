...
// list price shown struck-through; "price" is the SUBTOTAL — VAT (5%) is
// always added on top of it to get the total actually charged.
const PACKAGES = {
  REG:          { type: 'REGISTRATION', name: 'লাইফটাইম রেজিস্ট্রেশন ফি',       orig: 999,    price: 999 },
...
async function createInvoice(body) {
  ...
  // Every package's "price" is the subtotal; VAT (5%) is always added on
  // top of it to get the total actually charged.
  const discount = Math.max(0, pkg.orig - pkg.price);
  const vat = Math.round(pkg.price * 0.05);
  const total = pkg.price + vat;
  const baseAmount = pkg.orig;
  ...
  const invoice = await sbInsert('invoices', {
    ...
    amount: baseAmount, discount, vat, total_amount: total,
    status: 'PENDING',
  }, true);

...
exports.handler = async (event) => {
  ...
  if (action === 'verify') {
    ...
    if (row.verified) return reply(200, { verified: true, note: 'already verified' });
    if (row.attempts >= MAX_ATTEMPTS) return reply(200, { verified: false, error: 'অনেকবার ভুল হয়েছে — নতুন কোড নিন' });
